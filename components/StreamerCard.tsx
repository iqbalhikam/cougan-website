  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} whileHover={{ y: -5 }} className="group relative overflow-hidden rounded-xl bg-zinc-900 border border-white/5 hover:border-gold/50 transition-colors">
      <div className="aspect-square relative w-full overflow-hidden bg-zinc-800">
        <Image src={streamer.avatar} alt={streamer.name} fill className="object-cover transition-transform duration-500 group-hover:scale-110" />
        ...
        {streamer.status === 'live' && <div className="absolute top-4 right-4 px-2 py-1 bg-red-600 rounded text-xs font-bold text-white uppercase animate-pulse">LIVE</div>}
      </div>
