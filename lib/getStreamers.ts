  ...
  try {
    ...
    const updatedStreamers = await Promise.all(
      dbStreamers.map(async (streamer) => {
        ...
        try {
          // Construct URL: Use /@handle/live if it starts with @, otherwise /channel/ID/live
          const isHandle = streamer.channelId.startsWith('@');
          const url = isHandle ? `https://www.youtube.com/${streamer.channelId}/live` : `https://www.youtube.com/channel/${streamer.channelId}/live`;
